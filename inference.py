...
test_kpm_data = [
    {'ue_id': 1, 'dl_cqi': 15, 'ul_rsrp': 0.8, 'distance_ft': 30},
    {'ue_id': 1, 'dl_cqi': 14, 'ul_rsrp': 0.78, 'distance_ft': 30},
    {'ue_id': 1, 'dl_cqi': 13, 'ul_rsrp': 0.76, 'distance_ft': 30},
    {'ue_id': 1, 'dl_cqi': 13, 'ul_rsrp': 0.75, 'distance_ft': 30},
    {'ue_id': 1, 'dl_cqi': 12, 'ul_rsrp': 0.74, 'distance_ft': 30},
    {'ue_id': 1, 'dl_cqi': 12, 'ul_rsrp': 0.73, 'distance_ft': 30},
    {'ue_id': 1, 'dl_cqi': 11, 'ul_rsrp': 0.72, 'distance_ft': 30},
    {'ue_id': 1, 'dl_cqi': 11, 'ul_rsrp': 0.71, 'distance_ft': 30},
    {'ue_id': 1, 'dl_cqi': 10, 'ul_rsrp': 0.70, 'distance_ft': 30},
    {'ue_id': 1, 'dl_cqi': 10, 'ul_rsrp': 0.69, 'distance_ft': 30},
]
...
class FeatureProcessor:
    ...
    def process(self, data_point):
        ...
        temp_df['cqi_rolling_std'] = temp_df['dl_cqi'].rolling(window=5,min_periods=1).std().fillna(0)
...
def load_artifacts():
    ...
    global_threshold = threshold['global_threshold']
    print("Artifacts loaded")
    return model_ae, model_cls, ae_scaler, cls_scaler, scaler_divergence, global_threshold
...
def subscribe_to_kpm_data():
    ...
    while True:
        ...
        try:
            ...
            print("Value Error while decoding. Test message received")
            print(payload_str)

if __name__ ==  "__main__":
    model_ae, model_cls, ae_scaler, cls_scaler,divergence_scaler, thresholds = load_artifacts()
        ...
        try:
            ...
            with torch.no_grad():
                if current_distance in AUTOENCODER_DISTANCES:
                    ...
                    temp_df[FEATURES_TO_SCALE] = ae_scaler.transform(temp_df[FEATURES_TO_SCALE])
                    seq_tensor = torch.tensor(temp_df.values, dtype=torch.float32).unsqueeze(0).to(DEVICE)
                    reconstructed = model_ae(seq_tensor)
                    error = torch.mean((reconstructed - seq_tensor)**2).item()
                    threshold = thresholds.get(current_distance, global_threshold)
                    if error > threshold:
                        print(f"Anomaly Detected!!!!! Confidence: {error}")
                    else:
                        print("No Anomaly Detected. Confidence: {error}")

                else:
                    temp_df = sequence_df.copy()
                    temp_df[FEATURES_TO_SCALE] = cls_scaler.transform(temp_df[FEATURES_TO_SCALE])
                    seq_tensor = torch.tensor(temp_df.values, dtype=torch.float32).unsqueeze(0).to(DEVICE)
...
# lets test with a few data points:
